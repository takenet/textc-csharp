...
namespace Takenet.Textc.Types
{
    ...
    public class DoubleMetaphoneWordTokenType : ValueTokenTypeBase<string>
    {
        ...
        protected override bool HasMatch(string value, IRequestContext context, out string bestMatch)
        {
            ...
            var valueMetaphone = GenerateDoubleMetaphone(value);
            foreach (var validValue in GetValidValues(context))
            {
                var candidateMetaphone = GenerateDoubleMetaphone(validValue);
                if (candidateMetaphone.Equals(valueMetaphone))
                {
                    match = true;
        ...
        /// <returns></returns>
        private static string GenerateDoubleMetaphone(string self)
        {
            ...
            if (StartsWith(workingString, StringComparison.OrdinalIgnoreCase, "GN", "KN", "PN", "WR", "PS"))
            {
                current += 1;
            }
        ...
        private static bool IsVowel(char self)
        {
            return (self == 'A') || (self == 'E') || (self == 'I') || (self == 'O') || (self == 'U') || (self == 'Y');
        }

        private static bool StartsWith(string self, StringComparison comparison, params string[] strings)
        {
        ...
        private static bool StringAt(string self, int startIndex, params string[] strings)
        {
            if (startIndex < 0)
            {
                startIndex = 0;
